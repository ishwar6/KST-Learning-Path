            ...
            for the_chapter in the_chapters:
                ...
                if LEVEL_REF[level] == 0:
                    ...
                    nodes = Node.objects.filter(
                        state_node__topic__chapter=the_chapter).distinct()
                    print(nodes)
                    ...
                    for n in nodes:
                        ...
                        num_states_in_domain = max(
                            num_states_in_domain, num_mem)
                    print('max number of states in CHAPTER_PROFIENCY node',
                          num_states_in_domain)
    ...
    if user.is_authenticated:
        if request.method == 'GET':
            ...
            for c in chapters:
                ...
                print('this state is - -- - ', temp_state,
                      'for chapter - - - - ', c)
            ...
            a = 1
            print('all list of chapter_for_assessment are',
                  chapter_for_assessment)

            while(len(chapter_for_assessment) != 0):
                print('assessment chapter here is ', a)
                chapter_for_assessment = random.sample(
                    chapter_for_assessment, len(chapter_for_assessment))
                question, current_node = choose_question(
                    request, temp_states, chapter_for_assessment, 1)
                a = a+1
            ...
            print('very FIRST chapter of assessment here in get is',
                  chapter_for_assessment)
        ...
        if request.method == 'POST':
            ...
            if current_question.integer_type:
                integer_type_submission = str(
                    request.POST.get('integertype', False))
            ...
            if correct_answer_submission == 1:
                ...
                if question is None or number_of_question > 5:
                    ...
                    change_temp_state(
                        request, chapter_for_assessment[0], current_node)
                    number_of_question = 0
                ...
                this_q = 0
                (question, current_node) = choose_question(
                    request, temp_states, chapter_for_assessment, 0)
                if question is None or number_of_question > 5:
...
def choose_question(request, temp_states, chapter_for_assessment, next):
    ...
    if next == 1:   # if previous question submission is correct
        ...
        if len(outer_fringe_state) == 0:
            ...
            print('this has changed the temp node outer fringe to',
                  present_node, outer_fringe_state)
        ...
        if len(outer_fringe_state) == 0:
            ...
            questions        = Question.objects.filter(state=state_to_ask)
            change_temp_state(request, chapter, present_node[0])
            print('this has changed the temp node to',
                  present_node, outer_fringe_state)
            if questions.exists():
                return questions, present_node
            else:
...
def assessment_report(request):
    ...
    if user.is_authenticated:
        ...
        for new in new_chapter:
            ch_nodes = Node.objects.filter(
                state_node__topic__chapter=new).distinct()
            for nd in ch_nodes:
